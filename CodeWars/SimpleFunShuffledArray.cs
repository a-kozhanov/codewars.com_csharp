/*
https://www.codewars.com/kata/589573e3f0902e8919000109/csharp
...
but decided to store the sum he found in some random position of the original array which was a bad idea. 
Now he needs to cope with the second task, sorting the original array arr, 
...
It is guaranteed that there is an index i such that shuffled[i] = shuffled[0] + ... + shuffled[i - 1] + shuffled[i + 1] + ... + shuffled[n].
...
namespace CodeWars
{
    public class SimpleFunShuffledArray
    {
        public int[] ShuffledArray(int[] shuffled)
        {
            // var list = shuffled.ToList();
            // list.Remove(shuffled.Sum() / 2);
