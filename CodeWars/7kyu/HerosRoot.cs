/*
https://www.codewars.com/kata/55efecb8680f47654c000095/csharp
...
namespace CodeWars
{
    public class HerosRoot
    {
        ...
        {
            // return guess == (long) Math.Floor((guess + n / guess) / 2d)
            //     ? 1
            //     : 1 + IntRac(n, (long) Math.Floor((guess + n / guess) / 2d));

            // var x = (guess + n / guess) / 2;
            // if (Math.Abs(guess - x) >= 1)
            //     return IntRac(n, x) + 1;
            // return 1;

            var x = (guess + n / guess) / 2;
            if (Math.Abs(x - guess) < 1) return 1;
            return IntRac(n, x) + 1;
        }
    }
}