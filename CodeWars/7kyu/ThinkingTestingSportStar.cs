/*
https://www.codewars.com/kata/56dd927e4c9055f8470013a5/csharp
...
namespace CodeWars
{
    public class ThinkingTestingSportStar
    {
        public string Testit(string[] arr, string s)
        {
            return string.Concat(arr.Zip(s, (a, b) => a == "run" ? b == '_' ? b : '/' : b == '_' ? 'x' : '|'));
            //return string.Concat(arr.Select((x, i) => x == "run" ? s[i] == '_' ? '_' : '/' : s[i] == '_' ? 'x' : '|'));
            //return new string(arr.Zip(s, (x, y) => y == '_' ? x.Length == 3 ? y : 'x' : x.Length == 4 ? y : '/').ToArray());
        }
    }
}