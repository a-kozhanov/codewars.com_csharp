/*
https://www.codewars.com/kata/56d931ecc443d475d5000003/csharp
...
namespace CodeWars
{
    public class ThinkingTestingTrueOrFalse
    {
        public int Testit(int n)
        {
            return n > 0 ? n % 2 + Testit(n / 2) : 0;
            // return Convert.ToString(n, 2).Replace("0", "").Length;
            // return Convert.ToString(n, 2).Count(x => x == '1');
            // return (new BitArray(new[] {n})).Cast<bool>().Count(b => b);
            // return Convert.ToString(n, 2).Sum(s => int.Parse(s.ToString()));
        }
    }
}