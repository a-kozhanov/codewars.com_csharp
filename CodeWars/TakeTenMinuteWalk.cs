...
namespace CodeWars
{
    public class TakeTenMinuteWalk
    {
        ...
        {
            return walk.Length == 10 && walk.Sum(s => new Dictionary<string, int> {{"n", 1}, {"s", -1}, {"e", 2}, {"w", -2}}[s]) == 0;
            //return walk.Count(s => s == "n") == walk.Count(s => s == "s") && walk.Count(s => s == "e") == walk.Count(s => s == "w") && walk.Length == 10;
        }
    }
}