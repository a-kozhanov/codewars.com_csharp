    ...
    public class RandomTest
    {
        ...
        private static IEnumerable<TestCaseData> randomTestCases
        {
            get
            {
                ...
                {
                    List<char> validPin = new char[rnd.Next(2, 4) * 2].Select(_ => digits[rnd.Next(0, digits.Length)])
                        .ToList();
                    if (rnd.Next(0, 2) == 0)
                    {
                        validPin[rnd.Next(0, validPin.Count)] = (char) rnd.Next(32, 127);
                    }
                    ...
                    yield return new TestCaseData(pin).Returns(expected)
                        .SetDescription($"{pin} should be {(expected ? "valid" : "invalid")}");
                }
            }
        }
